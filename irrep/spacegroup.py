...
class SpaceGroup():
    ...
    def __init__(
        ...
        for isym, rot in enumerate(rotations):
            self.symmetries.append(SymmetryOperation(rot,
                                                translations[isym],
                                                self.Lattice,
                                                ind=isym+1,
                                                spinor=self.spinor,
        ...
        self.order = len(self.u_symmetries)  # To be redefined?
                ...
                if not all(np.isclose(k1, k2)):
                    raise RuntimeError(
                        "the kpoint {0} does not correspond to the point {1} ({2} in refUC / {3} in primUC) in the table".format(
                            ...
                            k1))
                tab[irr.name] = {}
                for i,(sym1,sym2) in enumerate(zip(self.u_symmetries, table.u_symmetries)):
                    try:
